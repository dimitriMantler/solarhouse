...
$(document).ready(function(){	
	...
	$("#capacity").ForceNumericOnly();
	$("#x").ForceNumericOnly();
	$("#y").ForceNumericOnly();
	$("#position").ForceNumericOnly();
	...
	$("#submit").click(function(){		
		...
		$.each(data,function(){
			...
			if(this.name == "lvl" || this.name == "price" || this.name == "capacity" || this.name == "position"){
...
/**
 * This function will send the login and createStorage data to the server
 */
function sendData(){
	//getVars
	var lvl, name, picpath, price, tooltip, capacity, x, y, position, email, password;
	lvl = $("#lvl").val();
	name = $("#name").val();
	picpath = $("#picpath").val();
	price = $("#price").val();
	...
	DEBUG.print("Storage:");
	DEBUG.print(jsonStorage);

	//Connect to server
	Connector.adminLogin(jsonAdmin,Utility.serverURL, //TODO test
	function(data){
		//Success handler from the RecieveData Request
		DEBUG.print("adminStorage:sendData(login)->Success");
		//check if data has not the failure flag set
		var msg = Connector.checkRecievedData(data);
		if(msg != "OK"){
			//Data has failure flag set!
			DEBUG.print("adminStorage:sendData(login)->Success->msg not OK: "+msg);
			showMessage(msg);
			return;
		}
		//Data was OK, so createStorage
		Connector.adminLogin(jsonStorage,Utility.serverURL,
		function(data){
			//Success handler from the RecieveData Request
			DEBUG.print("adminStorage:sendData(createStorage)->Success");
			//check if data has not the failure flag set
			var msg = Connector.checkRecievedData(data);
			if(msg != "OK"){
				//Data has failure flag set!
				DEBUG.print("adminStorage:sendData(createStorage)->Success->msg not OK: "+msg);
				showMessage(msg);
				return;
			}			
			...
			DEBUG.print("adminStorage(createStorage):sendData->Error");
			DEBUG.print(error);
			showMessage(Connector.translateErrorCode(error));
		});		
	},function(error){
		//Error handler from the RecieveData Request
		DEBUG.print("adminStorage(login):sendData->Error");
		DEBUG.print(error);
		showMessage(Connector.translateErrorCode(error));		
	});
}
